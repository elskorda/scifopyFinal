module vector_operations
	...
	interface operator(-)
 	...
 	interface operator(*)
		module procedure vector_dot_scalar
	end interface
...
type(vector) function vector_dot_scalar(v,s)
  type(vector), intent(in) :: v
  real(8), intent(in) :: s 
  
  vector_dot_scalar%c(1) = v%c(1)*s
  vector_dot_scalar%c(2) = v%c(2)*s
end function vector_dot_scalar
